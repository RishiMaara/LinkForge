...
export default async function DashboardPage() {
  ...
  const { data: links } = await supabase.from("links").select("*").eq("user_id", user!.id).order("created_at", { ascending: false });
    ...
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">Manage your short links and monitor their performance.</p>
        </div>
        <a
          href="/api/export"
          download
